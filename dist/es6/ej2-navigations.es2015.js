...
let HScroll = class HScroll extends Component {
    ...
    scrollHandler(e) {
        ...
        if (scrollLeft === 0) {
            ...
            let disEle = this.element.querySelector('.' + CLS_DISABLE);
            if (disEle) {
...
let Toolbar = class Toolbar extends Component {
        ...
        for (let el of [].slice.call(popupEle.children)) {
            ...
            if (elWidth < width || destroy) {
                ...
                if (this.tbarAlign) {
                    ...
                    index = this.tbarAlgEle[(pos + 's')].indexOf(el);
                    eleSplice = this.tbarAlgEle[(pos + 's')];
                    innerEle = this.element.querySelector('.' + CLS_ITEMS + ' .' + 'e-toolbar-' + pos);
                }
                let sepBeforePri = 0;
    ...
    removeItemByIndex(index, innerItems) {
        if (this.tbarEle[index] && innerItems[index]) {
            ...
            if (this.tbarAlign) {
                let indexAgn = this.tbarAlgEle[(this.items[eleIdx].align + 's')].indexOf(this.tbarEle[eleIdx]);
        ...
        let listBaseOptions = {
            ...
            itemCreated: (args) => {
                if (args.curData.separator) {
                    ...
                    let blankIcon = select('.e-blankicon', args.item);
                    if (blankIcon) {
                        remove(blankIcon);
                    }
                }
...
let Tab = class Tab extends Component {
    ...
    parseObject(items) {
        ...
        items.forEach((item, index) => {
            ...
            let id = ((tbCount === 0) ? index : (this.isReplace === true) ? index : tbCount + index);
            ...
            let wrap = createElement('div', { className: CLS_WRAP, innerHTML: tCont, attrs: { tabIndex: '-1' } });
            let attrObj = {
    ...
    addTab(items, index) {
        ...
        this.bdrLine.classList.add(CLS_HIDDEN);
        this.tbItems = select('.' + CLS_HEADER$1 + ' .' + CLS_TB_ITEMS, this.element);
        let tabItems = this.parseObject(items);
        items.forEach((item) => {
...
const POPUPSELECT = 'e-select';
const CONTENT = 'e-content';
const DOWNTAIL = 'e-downtail';
...
const SPINNER_SIZE = Browser.isDevice ? 16 : 14;
...
let TreeView = class TreeView extends Component {
    ...
    setTouchPopup(isEnabled) {
        if (isEnabled) {
            let popup = createElement('div', { className: POPUP, styles: 'display:none;' });
            let content = createElement('div', { className: CONTENT, attrs: { tabIndex: '-1' } });
            let icon = createElement('span', { className: ICON });
            ...
            let popup = select('.' + POPUP, this.element);
            if (popup) {
                detach(popup);
            }
        }
                ...
                if (classList$$1.contains(EXPANDABLE)) {
                    ...
                    if (!classList$$1.contains(PARENTITEM) && !classList$$1.contains(LISTITEM)) {
                        ...
                        if (Browser.isDevice && this.allowMultiSelection) {
                            ...
                            let txtWrap = closest(target, '.' + TEXTWRAP);
                            txtWrap = !txtWrap ? e.target : txtWrap;
                            let pos = txtWrap.getBoundingClientRect();
                            let posLeft = (e.clientX - pos.left) + txtWrap.offsetLeft - (popup.offsetWidth / 2);
                            let posTop = txtWrap.offsetTop - popup.offsetHeight - 10;
    ...
    getNodeData(currLi) {
        if (!isNullOrUndefined(currLi) && currLi.classList.contains(LISTITEM)) {
            ...
            let text = select('.' + LISTTEXT, currLi).textContent;
    ...
    createTextbox(liEle, e) {
        ...
        let eventArgs = this.getEditEvent(liEle, null);
        ...
        let inpEle = createElement('input', { attrs: { value: this.oldText } });
        ...
        if (this.fields.dataSource instanceof DataManager) {
            ...
            if (dropLi) {
                ...
                for (let i = 0; i < nodes.length; i++) {
                    let pid = nodes[i][this.fields.parentID];
...
let Sidebar = class Sidebar extends Component {
    ...
    addClass() {
        addClass([this.element.nextElementSibling], CONTENT$1);
    ...
    /**
     * Specifies the current open state of the sidebar component.
     * @returns boolean
     */
    isOpen() {
    ...
    setEnableGestures() {
        if (this.enableGestures) {
            ...
            this.swipeElementObject = new Touch(this.element, { swipe: this.enableGestureHandler.bind(this) });
        }
        else {
    ...
    changeType(type) {
        ...
        let margin = this.position === 'left' ? '0px 0px 0px ' + this.barWidth + 'px' : '0px ' + this.barWidth + 'px 0px 0px';
        let tempWidth = this.position === 'left' ? this.barWidth : -(this.barWidth);
    ...
    destroy() {
        ...
        this.element.style.transform = '';
        if (!isNullOrUndefined(this.element.nextElementSibling)) {
