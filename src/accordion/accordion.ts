...
export class Accordion extends Component<HTMLElement> implements INotifyPropertyChanged {
    ...
    private ctrlTemplate(): void {
       ...
       let innerEles: NodeList = this.element.children;
       let content: HTEle;
    ...
    private toggleIconGenerate(): HTEle {
      let tglIcon: HTEle = buildTag('div', {className: CLS_TOOGLEICN});
      let hdrColIcon: HTEle = buildTag('span', {className: CLS_COLLAPSEICN });
    ...
    private headerEleGenerate(): HTEle {
      let header: HTEle = buildTag('div', { className: CLS_HEADER, id: getUniqueID('acrdn_header') });
      let ariaAttr: { [key: string]: Str } = {
    ...
    private renderInnerItem(item: AccordionItemModel, index: number): HTEle {
      ...
      if (item.header) {
        ...
        let hdrEle: HTEle = buildTag('div', { className: CLS_HEADERCTN});
      ...
      if (item.iconCss) {
         let hdrIcnEle: HTEle = buildTag('div', { className: CLS_HEADERICN });
         let icon: HTEle = buildTag('span', { className: item.iconCss + ' e-icons' });
    ...
    private contentRendering(index: number): HTEle {
      ...
      let itemcnt: HTEle = buildTag('div', { className: CLS_CONTENT + ' ' + CLS_CTNHIDE, id: getUniqueID('acrdn_panel') });
      attributes(itemcnt, { 'aria-hidden': 'true' });
      let ctn: HTEle = buildTag('div', { className: CLS_CTENT });
